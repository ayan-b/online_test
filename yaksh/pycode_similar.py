...
class FuncNodeCollector(ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        ...
        self._last_node_lineno = max(getattr(node, 'lineno', -1),
                                     self._last_node_lineno)
    ...
    def visit_Compare(self, node):
        ...
        def _simple_nomalize(*ops_type_names):
            if ((node.ops and len(node.ops) == 1)
                    and (type(node.ops[0]).__name__ in ops_type_names)):
...
class FuncDiffInfo(object):
    """
    An object stores the result of candidate python code compared
    to referenced python code.
    """
    ...
    def plagiarism_percent(self):
        if self.total_count == 0:
            ...
            return self.plagiarism_count / float(self.total_count)

    def __str__(self):
        if (isinstance(self.info_ref, FuncInfo) and
                isinstance(self.info_candidate, FuncInfo)):
            return '{:<4.2}: ref {},'
            ' candidate {}'.format(
                            self.plagiarism_percent,
                            self.info_ref.func_name + '<' +
                            str(self.info_ref.func_node.lineno) + ':' +
                            str(self.info_ref.func_node.col_offset) + '>',
                            self.info_candidate.func_name + '<' +
                            str(self.info_candidate.func_node.lineno) + ':' +
                            str(self.info_candidate.func_node.col_offset) + '>'
                        )
...
class UnifiedDiff(object):
    """
    Line diff algorithm to formatted AST string lines, naive but efficiency,
...
def get_func_info(candidate_name, code_str):
...
def detect(pycode_string, diff_method=UnifiedDiff):
    if len(pycode_string) < 2:
        return []

    func_info_list = []

    if type(pycode_string) == dict:
        to_iterate = pycode_string.items()
    elif type(pycode_string) == list:
        to_iterate = enumerate(pycode_string)

    for candidate_name, code_str in to_iterate:
        func_info = get_func_info(candidate_name, code_str)
        func_info_list.append((candidate_name, func_info))
    ...
    for key_candidate, func_info_candidate in func_info_list[1:]:
        func_ast_diff_list = []
        for fi1 in func_info_ref:
            ...
            for fi2 in func_info_candidate:
                ...
                # entire function structure is plagiarized by candidate
                if dv == 0:
                    break
            ...
            func_diff_info.plagiarism_count = (
                func_diff_info.total_count - min_diff_value
                ) if min_diff_func_info else 0
            func_ast_diff_list.append(func_diff_info)
        func_ast_diff_list.sort(key=operator.attrgetter('plagiarism_percent'),
                                reverse=True)
        ast_diff_result.append((key_candidate, func_ast_diff_list))
...
def main():
    ...
    parser = ArgParser(description='A simple plagiarism detection tool for'
                       'python code')
    ...
    for index, func_ast_diff_list in results:
        ...
        print('{:.2f} % ({}/{}) of ref code structure is plagiarized'
              'by candidate.'.format(
                  sum_plagiarism_count / float(sum_total_count) * 100,
                  sum_plagiarism_count,
                  sum_total_count))
        print('candidate function plagiarism details (AST lines >= {}'
              ' and plagiarism percentage >= {}):'.format(
                  args.l,
                  args.p,
              ))
        output_count = 0
        for func_diff_info in func_ast_diff_list:
            if (len(func_diff_info.info_ref.func_ast_lines) >= args.l
                    and func_diff_info.plagiarism_percent >= args.p):
